...
export async function createNewAccessKeyForUser(username: string) {
    const iam = new IAM();
    const response = await iam.createAccessKey({
        UserName: username
    }).promise();
    return response.AccessKey;
}

export async function deleteAccessKey(username: string, oldAccessKeyId: string, newAccessKey: IAM.AccessKey) {
    ...
    const response = await iam.deleteAccessKey({
        UserName: username,
...
export async function updateSecret(secretName: string, secretValue: string, publicKey: { key_id: string, key: string }) {
    const octokit = new Octokit();
...
async function run() {
    ...
    try {
        ...
        const accessKeySecretName = core.getInput("ACCESS_KEY_ID_SECRET_NAME", { required: true });
        const secretAccessKeySecretName = core.getInput("SECRET_ACCESS_KEY_SECRET_NAME", { required: true });
        ...
        await deleteAccessKey(username, process.env.AWS_ACCESS_KEY_ID || "", newAccessKey);
